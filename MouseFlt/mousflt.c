...
#define MOUDEV_NAME L"\\Device\\mouflt"
#define  MOUSYM_NAME L"\\DosDevices\\mouflt"
...
NTSTATUS unloadWpdFlt(PDRIVER_OBJECT pDriver)
{
	...
	DbgPrint("[db]DriverEntry Unloading...\n");
...
NTSTATUS IoControlDispatch(PDEVICE_OBJECT pDevice, PIRP pIrp)
{
	...
	switch (CtlCode)
	{
	case CTL_START_MOUFLT:
	{
		DbgPrint("[db][DB]OVER\n");
		AttachDevice(g_pdriver, g_regpath);
		break;
	}
	case CTL_STOP_MOUFLT:
	{
...
NTSTATUS AttachDevice(PDRIVER_OBJECT pDriver, PUNICODE_STRING RegPatch)
{
	...
	RtlInitUnicodeString(&kbdName, L"\\Driver\\mouclass");
	...
	if (!NT_SUCCESS(status))
	{
		//DbgPrint("[db]Open KeyBoard Driver Failed\n");
		return status;
	}
	else
	{
	...
	while (targetDevice)
	{
		...
		if (!NT_SUCCESS(status))
		{
			//DbgPrint("[db]Create New FilterDevice Failed\n");
	...
	DbgPrint("[db]Create And Attach Finshed...\n");
	return status;
}
NTSTATUS StartFltWpd(PDRIVER_OBJECT pDriver, PUNICODE_STRING RegPath)
{
...
NTSTATUS DriverEntry(PDRIVER_OBJECT pDriveObject, PUNICODE_STRING Regpath)
{
	...
	UNICODE_STRING uDeciceName = RTL_CONSTANT_STRING(MOUDEV_NAME);
	UNICODE_STRING uSyb_Name = RTL_CONSTANT_STRING(MOUSYM_NAME);
